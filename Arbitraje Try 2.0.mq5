...
// ✅ CORRECCIÓN 1: Helpers con verificación de existencia de objetos
void ObjRect(string name, int x, int y, int w, int h, color clr, bool back=true, int alpha=200)
{
   ...
   ObjectSetInteger(0, name, OBJPROP_BACK,        back);  // ✅ CORREGIDO: Sin OBJPROP_SEND_TO_BACK
...
// ✅ CORRECCIÓN 2: Limpieza completa de objetos del panel
void fnDeletePanel()
{
   int total = ObjectsTotal(0, 0, -1);
   int pfxLen = StringLen(g_pfx);
   
   for(int i = total-1; i >= 0; i--)
   {
      string nm = ObjectName(0, i, 0, -1);
      if(StringLen(nm) >= pfxLen && StringSubstr(nm, 0, pfxLen) == g_pfx)
      {
         ObjectDelete(0, nm);
      }
   }
...
//-------------------------------------------------------------------
// fnDrawPanel — Panel HUD completo de 7 módulos
//-------------------------------------------------------------------
void fnDrawPanel(stThree &MxSmb[], ushort openThree)
{
   ...
   if(total <= 0) { fnDeletePanel(); return; }
   ...
   double nnStr   = fnNNStrength(avgNN)*100;
   double runtime = (double)(TimeCurrent()-g_startTime)/3600.0;
   
   string plSign  = (g_totalProfit >= 0) ? "+" : "";
   color  plColor = (g_totalProfit >= 0) ? g_theme.green_hi : g_theme.red_hi;
   color  opColor = (openPL >= 0)        ? g_theme.green_hi : g_theme.red_hi;
   
   //=================================================================
   // MÓDULO 0 — BARRA SUPERIOR (título + estado)
   //=================================================================
   int mod0y = py;
   int mod0h = 28;
   
   ObjRect(g_pfx+"hdr_bg",  px, mod0y, pw, mod0h, g_theme.bg_mid);
   ObjRect(g_pfx+"hdr_line",px, mod0y, pw, 2, g_theme.accent1);
   
   ObjLabel(g_pfx+"hdr_title", px+pad, mod0y+7,
            "▲  ARBITRAJE TRIANGULAR  PRO  v3.0", g_theme.accent1, 9, "Lucida Console");
   
   MqlDateTime dt; TimeToStruct(TimeCurrent(), dt);
   bool fridayLate = (dt.day_of_week==5 && dt.hour>=15);
   string stateStr = fridayLate ? "◉ PAUSADO (Viernes)" : "◉ ACTIVO";
   color  stateClr = fridayLate ? g_theme.red_hi : g_theme.green_hi;
   
   ObjLabel(g_pfx+"hdr_state", px+pw-pad, mod0y+7, stateStr, stateClr, 8, "Lucida Console", ANCHOR_RIGHT_UPPER);
   
   //=================================================================
   // MÓDULO 1 — MÉTRICAS CLAVE (4 tarjetas)
   //=================================================================
   int mod1y = mod0y + mod0h + 4;
   int mod1h = 54;
   int cw    = (pw - pad*5)/4;
   
   ObjRect(g_pfx+"m1_bg", px, mod1y, pw, mod1h, g_theme.bg_mid);
   
   // Tarjeta 1: P/L Total sesión
   int cx1 = px+pad;
   ObjRect(g_pfx+"c1_bg", cx1, mod1y+6, cw, mod1h-12, g_theme.bg_light);
   ObjRect(g_pfx+"c1_bar",cx1, mod1y+6, cw,  2, (g_totalProfit>=0?g_theme.green_hi:g_theme.red_hi));
   ObjLabel(g_pfx+"c1_lbl", cx1+4, mod1y+12, "P/L SESIÓN", g_theme.text_lo, 7, "Consolas");
   ObjLabel(g_pfx+"c1_val", cx1+cw/2, mod1y+26,
            plSign+DoubleToString(g_totalProfit,2), plColor, 9, "Consolas", ANCHOR_UPPER);
   
   // Tarjeta 2: P/L Abierto
   int cx2 = cx1+cw+pad;
   ObjRect(g_pfx+"c2_bg", cx2, mod1y+6, cw, mod1h-12, g_theme.bg_light);
   ObjRect(g_pfx+"c2_bar",cx2, mod1y+6, cw,  2, (openPL>=0?g_theme.green_hi:g_theme.red_hi));
   ObjLabel(g_pfx+"c2_lbl", cx2+4, mod1y+12, "P/L ABIERTO", g_theme.text_lo, 7, "Consolas");
   ObjLabel(g_pfx+"c2_val", cx2+cw/2, mod1y+26,
            (openPL>=0?"+":"")+DoubleToString(openPL,2), opColor, 9, "Consolas", ANCHOR_UPPER);
   
   // Tarjeta 3: Triángulos
   int cx3 = cx2+cw+pad;
   ObjRect(g_pfx+"c3_bg", cx3, mod1y+6, cw, mod1h-12, g_theme.bg_light);
   ObjRect(g_pfx+"c3_bar",cx3, mod1y+6, cw,  2, g_theme.accent2);
   ObjLabel(g_pfx+"c3_lbl", cx3+4, mod1y+12, "TRIÁNGULOS", g_theme.text_lo, 7, "Consolas");
   ObjLabel(g_pfx+"c3_val", cx3+cw/2, mod1y+26,
            IntegerToString(openThree)+"/"+IntegerToString(total), g_theme.accent1, 9, "Consolas", ANCHOR_UPPER);
   
   // Tarjeta 4: Win Rate
   int cx4 = cx3+cw+pad;
   color wrColor = (winRate>=55) ? g_theme.green_hi : (winRate>=45 ? g_theme.accent1 : g_theme.red_hi);
   ObjRect(g_pfx+"c4_bg", cx4, mod1y+6, cw, mod1h-12, g_theme.bg_light);
   ObjRect(g_pfx+"c4_bar",cx4, mod1y+6, cw,  2, wrColor);
   ObjLabel(g_pfx+"c4_lbl", cx4+4, mod1y+12, "WIN RATE",  g_theme.text_lo, 7, "Consolas");
   ObjLabel(g_pfx+"c4_val", cx4+cw/2, mod1y+26,
            DoubleToString(winRate,1)+"%", wrColor, 9, "Consolas", ANCHOR_UPPER);
   
   //=================================================================
   // MÓDULO 2 — RED NEURONAL + IA (izquierda)
   //=================================================================
   int mod2y  = mod1y + mod1h + 4;
   int mod2h  = 110;
   int col1x  = px;
   
   ObjRect(g_pfx+"m2_bg", col1x, mod2y, sw+pad, mod2h, g_theme.bg_mid);
   ObjRect(g_pfx+"m2_hdr",col1x, mod2y, sw+pad, 18, g_theme.bg_light);
   ObjLabel(g_pfx+"m2_ttl",col1x+pad, mod2y+4, "◈  RED NEURONAL / IA", g_theme.accent1, 8, "Consolas");
   
   int ry = mod2y + 22;
   string nnSt = inUseNeuralNet ? (g_nnReady ? "ENTRENADA ✓" : "PENDIENTE…") : "DESACTIVADA";
   color  nnCl = inUseNeuralNet ? (g_nnReady ? g_theme.green_hi : g_theme.accent2) : g_theme.text_lo;
   
   ObjLabel(g_pfx+"m2_l1", col1x+pad, ry,    "Estado NN:", g_theme.text_lo,  7, "Consolas"); ry+=lh;
   ObjLabel(g_pfx+"m2_v1", col1x+pad+80, ry-lh, nnSt, nnCl, 7, "Consolas");
   ObjLabel(g_pfx+"m2_l2", col1x+pad, ry,    "TF Entrena:", g_theme.text_lo, 7, "Consolas");
   ObjLabel(g_pfx+"m2_v2", col1x+pad+80, ry, EnumToString(inNNTimeframe), g_theme.text_hi, 7, "Consolas"); ry+=lh;
   ObjLabel(g_pfx+"m2_l3", col1x+pad, ry,    "Precisión:", g_theme.text_lo,  7, "Consolas");
   ObjLabel(g_pfx+"m2_v3", col1x+pad+80, ry, DoubleToString(g_nnAccuracy*100,1)+"%", g_theme.text_hi, 7, "Consolas"); ry+=lh;
   ObjLabel(g_pfx+"m2_l4", col1x+pad, ry,    "Loss:", g_theme.text_lo,       7, "Consolas");
   ObjLabel(g_pfx+"m2_v4", col1x+pad+80, ry, DoubleToString(g_nnLoss,4), g_theme.text_hi, 7, "Consolas"); ry+=lh;
   ObjLabel(g_pfx+"m2_l5", col1x+pad, ry,    "Muestras:", g_theme.text_lo,   7, "Consolas");
   ObjLabel(g_pfx+"m2_v5", col1x+pad+80, ry, DoubleToString(g_nnSamples,0), g_theme.text_hi, 7, "Consolas"); ry+=lh;
   ObjLabel(g_pfx+"m2_l6", col1x+pad, ry,    "Prob prom:", g_theme.text_lo,  7, "Consolas");
   color nnPrColor = (avgNN>inNNBuyThr) ? g_theme.green_hi : (avgNN<inNNSellThr ? g_theme.red_hi : g_theme.accent1);
   ObjLabel(g_pfx+"m2_v6", col1x+pad+80, ry, DoubleToString(avgNN*100,1)+"%", nnPrColor, 7, "Consolas"); ry+=lh;
   
   string trainStr = (g_nnLastTrain>0) ? TimeToString(g_nnLastTrain,TIME_DATE|TIME_MINUTES) : "N/A";
   ObjLabel(g_pfx+"m2_tr", col1x+pad, ry, "Train: "+trainStr, g_theme.text_lo, 6, "Consolas");
   
   //=================================================================
   // MÓDULO 3 — ESTADÍSTICAS IA + CUENTA (derecha)
   //=================================================================
   // ✅ CORRECCIÓN 3: Alineación de columnas corregida
   int col2x = col1x + sw + pad*2;
   
   ObjRect(g_pfx+"m3_bg", col2x, mod2y, sw+pad, mod2h, g_theme.bg_mid);
   ObjRect(g_pfx+"m3_hdr",col2x, mod2y, sw+pad, 18, g_theme.bg_light);
   ObjLabel(g_pfx+"m3_ttl",col2x+pad, mod2y+4, "◈  ESTADÍSTICAS / CUENTA", g_theme.accent1, 8, "Consolas");
   
   int ay = mod2y+22;
   ObjLabel(g_pfx+"m3_l1", col2x+pad, ay, "IA Score:", g_theme.text_lo,  7, "Consolas");
   color scColor = (avgScore>0) ? g_theme.green_hi : (avgScore<-0.1 ? g_theme.red_hi : g_theme.accent1);
   ObjLabel(g_pfx+"m3_v1", col2x+pad+80, ay, DoubleToString(avgScore,3), scColor, 7, "Consolas"); ay+=lh;
   
   ObjLabel(g_pfx+"m3_l2", col2x+pad, ay, "Confianza:", g_theme.text_lo, 7, "Consolas");
   ObjLabel(g_pfx+"m3_v2", col2x+pad+80, ay, DoubleToString(avgConf,1)+"%", g_theme.text_hi, 7, "Consolas"); ay+=lh;
   
   ObjLabel(g_pfx+"m3_l3", col2x+pad, ay, "Trades:", g_theme.text_lo,    7, "Consolas");
   ObjLabel(g_pfx+"m3_v3", col2x+pad+80, ay, IntegerToString(totTrades)+
            " (W:"+IntegerToString(totWins)+" L:"+IntegerToString(totLosses)+")", g_theme.text_hi, 7, "Consolas"); ay+=lh;
   
   ObjLabel(g_pfx+"m3_l4", col2x+pad, ay, "Balance:", g_theme.text_lo,   7, "Consolas");
   ObjLabel(g_pfx+"m3_v4", col2x+pad+80, ay, DoubleToString(AccountInfoDouble(ACCOUNT_BALANCE),2), g_theme.text_hi, 7, "Consolas"); ay+=lh;
   
   ObjLabel(g_pfx+"m3_l5", col2x+pad, ay, "Equity:", g_theme.text_lo,    7, "Consolas");
   double equity = AccountInfoDouble(ACCOUNT_EQUITY);
   double bal    = AccountInfoDouble(ACCOUNT_BALANCE);
   color eqColor = (equity>=bal) ? g_theme.green_hi : g_theme.red_hi;
   ObjLabel(g_pfx+"m3_v5", col2x+pad+80, ay, DoubleToString(equity,2), eqColor, 7, "Consolas"); ay+=lh;
   
   ObjLabel(g_pfx+"m3_l6", col2x+pad, ay, "Margen libre:", g_theme.text_lo, 7, "Consolas");
   ObjLabel(g_pfx+"m3_v6", col2x+pad+80, ay, DoubleToString(AccountInfoDouble(ACCOUNT_MARGIN_FREE),2), g_theme.text_hi, 7, "Consolas"); ay+=lh;
   
   ObjLabel(g_pfx+"m3_rt", col2x+pad, ay, "Runtime: "+DoubleToString(runtime,1)+"h", g_theme.text_lo, 6, "Consolas");
   
   //=================================================================
   // MÓDULO 4 — MEJOR OPORTUNIDAD ACTUAL
   //=================================================================
   int mod4y = mod2y + mod2h + 4;
   int mod4h = 80;
   
   ObjRect(g_pfx+"m4_bg",  px, mod4y, pw, mod4h, g_theme.bg_mid);
   ObjRect(g_pfx+"m4_hdr", px, mod4y, pw, 18, g_theme.bg_light);
   ObjLabel(g_pfx+"m4_ttl",px+pad, mod4y+4, "★  MEJOR OPORTUNIDAD ACTUAL", g_theme.accent1, 8, "Consolas");
   
   if(bestIdx >= 0)
   {
      double eb_best = MxSmb[bestIdx].PLBuy  - MxSmb[bestIdx].spreadbuy;
      double es_best = MxSmb[bestIdx].PLSell - MxSmb[bestIdx].spreadsell;
      bool   isBuy   = (eb_best >= es_best);
      double edge    = MathMax(eb_best, es_best);
      ...
      color  edgeClr = (edge > 0) ? g_theme.green_hi : g_theme.red_hi;
      
      ObjLabel(g_pfx+"m4_dir",  px+pad,      mod4y+23, dirStr, dirClr,  10, "Lucida Console");
      ObjLabel(g_pfx+"m4_sym",  px+pad+65,   mod4y+25, symStr, g_theme.text_hi, 8, "Consolas");
      ObjLabel(g_pfx+"m4_edge", px+pad,      mod4y+43, "Edge: "+DoubleToString(edge,2)+
               "   NN: "+DoubleToString(MxSmb[bestIdx].nnProb*100,1)+"%"+
               "   Confianza: "+DoubleToString(MxSmb[bestIdx].aiConfidence,1)+"%"+
               "   MM: "+(MxSmb[bestIdx].mmBias==1?"▲":(MxSmb[bestIdx].mmBias==-1?"▼":"—")),
               edgeClr, 7, "Consolas");
      
      // ✅ CORRECCIÓN 4: Barra de edge con protección contra división por cero
      int barMax = pw - pad*2;
      double edgeNorm = (edge > 0) ? fnClamp(edge / 10.0, 0.0, 1.0) : 0.0;
      int    barW    = (int)(barMax * edgeNorm);
      if(barW < 2) barW = 2;
      if(barW > barMax) barW = barMax;
      
      ObjRect(g_pfx+"m4_barBg", px+pad,    mod4y+60, barMax, 6, g_theme.bg_light);
      ObjRect(g_pfx+"m4_barFg", px+pad,    mod4y+60, barW,   6, edgeClr);
      ObjLabel(g_pfx+"m4_spread", px+pw-pad, mod4y+63,
               "Spread costo: "+DoubleToString((MxSmb[bestIdx].spreadbuy+MxSmb[bestIdx].spreadsell)/2,2),
               g_theme.text_lo, 6, "Consolas", ANCHOR_RIGHT_UPPER);
   }
   else
   {
      ObjLabel(g_pfx+"m4_none", px+pad, mod4y+35, "Sin oportunidades con edge positivo…", g_theme.text_lo, 8, "Consolas");
   }
   
   //=================================================================
   // MÓDULO 5 — TOP 5 OPORTUNIDADES
   //=================================================================
   int mod5y = mod4y + mod4h + 4;
   int mod5h = 18 + 5*(lh+2) + 4;
   
   ObjRect(g_pfx+"m5_bg",  px, mod5y, pw, mod5h, g_theme.bg_mid);
   ObjRect(g_pfx+"m5_hdr", px, mod5y, pw, 18, g_theme.bg_light);
   ObjLabel(g_pfx+"m5_ttl",px+pad, mod5y+4, "▦  TOP 5 OPORTUNIDADES", g_theme.accent1, 8, "Consolas");
   
   // Cabecera columnas
   int ty = mod5y + 20;
   ObjLabel(g_pfx+"m5_ch0", px+pad,          ty, "#", g_theme.text_lo, 6, "Consolas");
   ObjLabel(g_pfx+"m5_ch1", px+pad+16,        ty, "DIRECCIÓN", g_theme.text_lo, 6, "Consolas");
   ObjLabel(g_pfx+"m5_ch2", px+pad+72,        ty, "TRIÁNGULO",          g_theme.text_lo, 6, "Consolas");
   ObjLabel(g_pfx+"m5_ch3", px+pw-130,        ty, "EDGE", g_theme.text_lo, 6, "Consolas", ANCHOR_RIGHT_UPPER);
   ObjLabel(g_pfx+"m5_ch4", px+pw-75,         ty, "NN %", g_theme.text_lo, 6, "Consolas", ANCHOR_RIGHT_UPPER);
   ObjLabel(g_pfx+"m5_ch5", px+pw-pad,        ty, "STATUS", g_theme.text_lo, 6, "Consolas", ANCHOR_RIGHT_UPPER);
   
   ty += lh;
   
   // ✅ CORRECCIÓN 5: Verificación de índice de array en Top 5
   for(int t=0; t<5; t++)
   {
      string rn = IntegerToString(t)+"_";
      int idx = topIdx[t];
      color rowBg = (t%2==0) ? g_theme.bg_light : g_theme.bg_mid;
      
      ObjRect(g_pfx+"m5_rbg"+rn, px+pad-2, ty-1, pw-pad*2+4, lh+1, rowBg);
      
      if(idx < 0 || idx >= total)  // ✅ Verificación de límites
      {
         ObjLabel(g_pfx+"m5_r"+rn, px+pad, ty, IntegerToString(t+1)+"  —", g_theme.text_lo, 6, "Consolas");
         ty += lh+2; continue;
      }
      ...
      string stat= (MxSmb[idx].status==0) ? "LIBRE" : (MxSmb[idx].status==1?"ABRND":(MxSmb[idx].status==2?"OPEN":"CLOSE"));
      color  sc2 = (MxSmb[idx].status==2) ? g_theme.green_hi : (MxSmb[idx].status==3 ? g_theme.red_hi : g_theme.accent2);
      color  ec2 = (e>0) ? g_theme.green_hi : g_theme.red_hi;
      
      ObjLabel(g_pfx+"m5_ri"+rn, px+pad,     ty, IntegerToString(t+1), g_theme.text_lo, 6, "Consolas");
      ObjLabel(g_pfx+"m5_rd"+rn, px+pad+16,  ty, dir, dc, 6, "Consolas");
      ObjLabel(g_pfx+"m5_rs"+rn, px+pad+72,  ty, sym, g_theme.text_hi, 6, "Consolas");
      ObjLabel(g_pfx+"m5_re"+rn, px+pw-130,  ty, DoubleToString(e,2), ec2, 6, "Consolas", ANCHOR_RIGHT_UPPER);
      ObjLabel(g_pfx+"m5_rn"+rn, px+pw-75,   ty, DoubleToString(MxSmb[idx].nnProb*100,1), g_theme.text_hi, 6, "Consolas", ANCHOR_RIGHT_UPPER);
      ObjLabel(g_pfx+"m5_rt"+rn, px+pw-pad,  ty, stat, sc2, 6, "Consolas", ANCHOR_RIGHT_UPPER);
      
      ty += lh+2;
   }
   
   //=================================================================
   // MÓDULO 6 — TRIÁNGULOS ABIERTOS ACTIVOS
   //=================================================================
   int mod6y  = mod5y + mod5h + 4;
   int openCnt= 0;
   for(int i=0;i<total;i++) if(MxSmb[i].status==2) openCnt++;
   
   // ✅ CORRECCIÓN 6: Altura del módulo 6 corregida
   int mod6h  = (openCnt>0) ? 18 + openCnt*(lh+4) + 12 : 40;
   
   ObjRect(g_pfx+"m6_bg",  px, mod6y, pw, mod6h, g_theme.bg_mid);
   ObjRect(g_pfx+"m6_hdr", px, mod6y, pw, 18, g_theme.bg_light);
   ObjRect(g_pfx+"m6_accent", px, mod6y, 3, mod6h, g_theme.green_hi);
   
   ObjLabel(g_pfx+"m6_ttl",px+pad+2, mod6y+4,
            "⬡  POSICIONES ABIERTAS  ("+IntegerToString(openCnt)+")", g_theme.accent1, 8, "Consolas");
   
   int oy = mod6y + 20;
   
   if(openCnt == 0)
      ObjLabel(g_pfx+"m6_none", px+pad+4, oy+2, "Sin posiciones abiertas actualmente", g_theme.text_lo, 7, "Consolas");
   else
   {
      int oc=0;
      for(int i=total-1; i>=0; i--)
      {
         ...
         string rn = IntegerToString(oc)+"_";
         color rb = (oc%2==0) ? g_theme.bg_light : g_theme.bg_mid;
         
         ObjRect(g_pfx+"m6_rbg"+rn, px+pad-2, oy-1, pw-pad*2+4, lh+1, rb);
         
         color pc = (MxSmb[i].pl>=0) ? g_theme.green_hi : g_theme.red_hi;
         string plStr = (MxSmb[i].pl>=0?"+":"")+DoubleToString(MxSmb[i].pl,2);
         string timeOpen = TimeToString(MxSmb[i].timeopen, TIME_MINUTES|TIME_SECONDS);
         
         ObjLabel(g_pfx+"m6_sym"+rn, px+pad+4, oy,
                  MxSmb[i].smb1.name+"+"+MxSmb[i].smb2.name+"+"+MxSmb[i].smb3.name,
                  g_theme.text_hi, 7, "Consolas");
         ObjLabel(g_pfx+"m6_pl"+rn,  px+pw-pad-70, oy, plStr, pc, 7, "Consolas");
         ObjLabel(g_pfx+"m6_tm"+rn,  px+pw-pad, oy, timeOpen, g_theme.text_lo, 6, "Consolas", ANCHOR_RIGHT_UPPER);
   ...
   //=================================================================
   // MÓDULO 7 — BARRA INFERIOR (timestamp + broker)
   //=================================================================
   int mod7y = mod6y + mod6h + 4;
   int mod7h = 20;
   
   // ✅ CORRECCIÓN 7: ChartHeightInPixels no existe en MQL5, usar ChartGetInteger
   long chartHeight = ChartGetInteger(0, CHART_HEIGHT_IN_PIXELS);
   if(mod7y + mod7h > chartHeight)
      mod7y = (int)(chartHeight - mod7h - 4);
   
   ObjRect(g_pfx+"ftr_bg",   px, mod7y, pw, mod7h, g_theme.bg_dark);
   ObjRect(g_pfx+"ftr_line", px, mod7y+mod7h-2, pw, 2, g_theme.accent2);
   
   ObjLabel(g_pfx+"ftr_time",px+pad, mod7y+5,
            "[T] "+TimeToString(TimeCurrent(),TIME_DATE|TIME_MINUTES|TIME_SECONDS),
            g_theme.text_lo, 6, "Consolas");
   
   ObjLabel(g_pfx+"ftr_acc", px+pw-pad, mod7y+5,
